...
class FromTo_Cntl {
	...
	init(f_path: String, t_path: String) {
	
		print("f_path: \(f_path) t_path: \(t_path)")
		...
		if (fileManager.fileExists(atPath: t_path, isDirectory:&isDir) == false) {
		
			str_Error = "To がディレクトリで無い！！"
			str_Status = "Abend"
			bool_WaitFlag = false
